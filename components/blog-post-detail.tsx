...
export const BlogPostDetail: React.FC<BlogPostDetailProps> = ({ post, onBack }) => {
  return (
    <div className="animate-fade-in-up">
      <button
        onClick={onBack}
        className="mb-8 flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors px-4 py-2 rounded-full hover:bg-white/5 w-fit"
      >
      ...
      <article className="max-w-3xl mx-auto">
        <header className="mb-12 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {post.tags.map(tag => (
              <span key={tag} className="px-3 py-1 rounded-full border border-white/10 bg-white/5 text-xs text-zinc-300">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="w-full aspect-video rounded-3xl overflow-hidden mb-12 border border-white/5">
        ...
        <div className="flex justify-between items-center">
          ...
          <button className="p-3 rounded-full hover:bg-white/10 transition-colors text-zinc-400 hover:text-white">
